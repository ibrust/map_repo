  ...
  import * as ArcGIS from './ArcGIS.js';
    ...
    var intersection_SQL_input_1 = document.getElementById("intersection_SQL_input_1");
    var intersection_SQL_input_2 = document.getElementById("intersection_SQL_input_2");
    var buffer_SQL_input = document.getElementById("buffer_SQL_input");
    var search_SQL_input = document.getElementById("search_SQL_input");
...
function fill_attribute_table(input_page, reload_buttons, use_graphics, from_search_button){
  ...
  if (reload_buttons == true){  // it's called if a page number is changed, a layer changes, a service changes, with filter by extent or mouse select, etc.
    ...
    if (State.filterActive.get() == true){
      initialize_page_buttons(1, true, State.MapGraphics.enable_graphics);
    }
    else if (State.MapGraphics.enable_graphics == true){
      initialize_page_buttons(input_page, true, State.MapGraphics.enable_graphics);
    }
    else{
      initialize_page_buttons(1, true, State.MapGraphics.enable_graphics);
    }
...
function initialize_page_buttons(starting_page, from_refill_table, use_graphics){
  ...
  function create_buttons(feature_count){
    ...
    // now you'll call adjust_page_buttons one time, which will initially display buttons 1-7 and hide the rest 
...
function adjust_page_buttons(newly_clicked_page, total_pages, from_refill_table, use_graphics){
	...
	let layer_id = table_dataset_selector.options[table_dataset_selector.selectedIndex].id;
...
function change_results_count(count){
  ...
  if (State.filterActive.get() == false){
    initialize_page_buttons(State.UIProperties.current_page, false, State.MapGraphics.enable_graphics);
  }
...
function initialize_listeners(){ 
  ...
  // listener for handling submissions from various toolboxes
	intersection_submit_button.addEventListener("click", function(){
		intersect_function();
	});
...
function intersect_function(){
  ...
  if (intersect_error_div.innerHTML != ""){
	...
	var SQL_query_one = intersection_SQL_input_1.value; 
	var SQL_query_two = intersection_SQL_input_2.value;
...
function buffer_function(){
    ...
    var SQL_query = buffer_SQL_input.value;
...
function search_function(){
    ...
    var SQL_query = search_SQL_input.value;
    if (SQL_query == ""){
      SQL_query = "1=1";
    }
...
function table_select_handler(e){
  ...
  if (State.filterActive.get() == true){
    ...
    fill_attribute_table(1, true, State.MapGraphics.enable_graphics);
  }
}
...
function click_select_function(e){
  ...
  Promise.all(new_promises_array).then(function(array_results){
    ...
    .then(function(){
      ...
      fill_attribute_table(page_number, true, State.MapGraphics.enable_graphics);
    });
